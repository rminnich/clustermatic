...
void Usage(char *arg0) {
    printf(
"usage: %s [options] nodenumber command\n"
"       %s -a [options] command\n"
"       %s -A [options] command\n"
...
const char *which(const char *progname) {
    ...
    ptr = getenv("PATH");
    if (!ptr) ptr = ":/bin:/usr/bin"; /* same default as execvp */
    end = ptr - 1;
    while (*end) {
	ptr = end+1;
	end = strchr(ptr, ':');
	...
	if (!path_try) return 0; /* out of memory */

	/* and empty path element means ".", not "/" */
	if (end-ptr == 0)
	    sprintf(path_try, "./%s", progname);
	else
	    sprintf(path_try, "%.*s/%s", (int)(end-ptr), ptr, progname);
...
int do_line_buff_io(int nodenum, struct output_buffer *io, int flags) {
	...
	if (io->bytes) {
	    if (flags & IO_PREFIX) write(io->outfd, prefix, strlen(prefix));
	    write(io->outfd, io->buf, io->bytes);
	    if (flags & IO_PREFIX) write(io->outfd, "\n", 1);
	    io->bytes = 0;
	}
	if (io->infd >= 3)  close(io->infd);
	io->infd = -1;
	if (io->outfd >= 3) close(io->outfd);
	io->outfd = -1;
	return -1;
    }
    io->bytes += r;
...
void cleanup_children(void) {
	...
	for (i=0; i < num_nodes; i++) {
	    if (nodes[i].pid == pid) {
		if (!nodes[i].alive)
		    fprintf(stderr,"Dead node picked up in sigchld_handler\n");
		if (nodes[i].cnum) {
		     ...
		     * it very long. */
		    outstanding_connections -= nodes[i].cnum;
		...
		if (WIFEXITED(status)) {
		    ...
		    fprintf(stderr, "bpsh: Child process on node %d exited"
			    " abnormally.\n", nodes[i].node);
...
void forward_io_init(int flags, struct bproc_io_t *io, int iolen) {
    ...
    if (!(flags & IO_LINEBUFF)) buffer = malloc(buffer_size);
	...
	nodes[i].io[0].infd  = -1;
	nodes[i].io[0].outfd = io[1].type==BPROC_IO_SOCKET ? STDOUT_FILENO:-1;
	nodes[i].io[0].bytes = 0;
	nodes[i].io[0].buf   = (flags&IO_LINEBUFF)?malloc(buffer_size):buffer;

	nodes[i].io[1].infd  = -1;
	nodes[i].io[1].outfd = io[2].type==BPROC_IO_SOCKET ? STDERR_FILENO:-1;
	nodes[i].io[1].bytes = 0;
	nodes[i].io[1].buf   = (flags&IO_LINEBUFF)?malloc(buffer_size):buffer;
...
void forward_io_accept(void) {
	...
	if (r == 1) {
	    sasize = sizeof(sa);
	    accept_fdlist[accept_nfds] = accept(accept_sockfd, &sa, &sasize);
...
void forward_io_new_fd(int fd) {
	...
	for (i=0; i < num_nodes; i++) {
	    if (nodes[i].pid == pid) {
		if (nodes[i].cnum == 0) {
		    fprintf(stderr, "too many connections from"
			    " pid %d (rfd=%d)\n", pid, rfd);
		    close(fd);
		    break;
		}
		...
		case STDIN_FILENO:
		    set_non_block(fd);
		    if (do_input_buffer_add(&inbuf, i, fd)==0)
			io_to_do++;
		    else
			close(fd);
		    if (nodes[i].alive)
		...
		case STDERR_FILENO:
		    ...
		    break;
		default:
		    fprintf(stderr, "bpsh: %d: bad remote fd"
			    " number: %d\n", pid, rfd);
		    close(fd);
		}
		break;
...
void forward_io(int sockfd, int flags, struct bproc_io_t *io, int iolen,
	...
	if (inbuf.infd != -1 && inbuf.bytes < buffer_size) {
	    ...
	    if (max < inbuf.infd) max = inbuf.infd;
	}
	for (i=0; i < num_nodes; i++) {
	    if (inbuf.outfds[i] != -1) {
		FD_SET(inbuf.outfds[i], rset);
		if (inbuf.readptrs[i] < (inbuf.buf + inbuf.bytes))
		    FD_SET(inbuf.outfds[i], wset);
		if (max < inbuf.outfds[i]) max = inbuf.outfds[i];
	    }
	}
	...
	if (flags & IO_SEQUENTIAL && current_node < num_nodes) {
		...
		if (nodes[current_node].io[j].infd != -1 &&
		    ...
		    FD_SET(nodes[current_node].io[j].infd, rset);
		    if (max < nodes[current_node].io[j].infd)
			max = nodes[current_node].io[j].infd;
		}
	    }
	} else {
		...
		for (j=0; j < 2; j++) {
			...
			FD_SET(nodes[i].io[j].infd, rset);
			if (max < nodes[i].io[j].infd)
			    max = nodes[i].io[j].infd;
		    }
		}
	    }
	}
	...
	if (r > 0) {
		...
		int numclosed;
		if (do_input_buffer_read(&inbuf, &numclosed) != 0)
		    io_to_do -= numclosed;
	    }
	    for (i=0; i < num_nodes; i++) {
		...
		if (inbuf.outfds[i] != -1 && FD_ISSET(inbuf.outfds[i], rset)){
		    do_input_buffer_close1(&inbuf, i);
		    io_to_do--;
		}
		if (inbuf.outfds[i] != -1 && FD_ISSET(inbuf.outfds[i], wset))
		    if (do_input_buffer_write(&inbuf, i) != 0) {
			io_to_do--;
		    }
	    }
	    /* Do output fds */
		...
		for (j=0; j < 2; j++) {
		    if (nodes[i].io[j].infd != -1 &&
			FD_ISSET(nodes[i].io[j].infd, rset)) {
			if (flags & IO_LINEBUFF) {
			    if (do_line_buff_io(nodes[i].node,
						&nodes[i].io[j], flags) != 0){
				io_to_do --;
			    }
			} else {
			    if (do_unbuff_io(&nodes[i].io[j]) != 0) {
				io_to_do --;
			    }
			}
		    }
	...
	if (r == 0) {
		...
		for (i=0; i < num_nodes; i++)
			...
			fprintf(stderr, "bpsh: timeout connection from"
				" node %d\n", nodes[i].node);
			outstanding_connections -= nodes[i].cnum;
		    }
	    }
	...
	if ((flags & IO_SEQUENTIAL) && !late_connections) {
		...
		current_node++;
		if (flags & IO_DIVIDERS && current_node < num_nodes) {
		    printf("%d\t-------------------------------------------"
			   "--------------------------\n",
			   nodes[current_node].node);
		    fflush(stdout);
...
int bproc_addr2node(struct bproc_node_set_t *ns, char *addr) {
    ...
    for (i = 0 ; i < n; i++) {
	struct sockaddr_in *sin = (struct sockaddr_in *)&nodes.node[i].addr;
...
int main(int argc, char *argv[]) {
    ...
    while(optind<argc) {
		...
		case '1': /* the user wanted to run on node -1 */
			...
			non_option_args++;
			break;
		case 'l': /* silently ignore */ break;
		/* Node selection */
		case 'a': all_up = 1;    non_option_args = 1; break;
		case 'A': all_avail = 1; non_option_args = 1; break;
		/* IO forwarding options */
		case 'n':
		    nullfd = open("/dev/null", O_RDONLY);
		    if (nullfd == -1) { perror("/dev/null"); exit(1); }
		...
		case 'N':
		    ...
		    break;
		case 'L': flags |= IO_LINEBUFF; break;
		case 'p': flags |= (IO_LINEBUFF | IO_PREFIX); break;
		case 's': flags |= IO_SEQUENTIAL; break;
		case 'd': flags |= (IO_DIVIDERS | IO_SEQUENTIAL); break;
		case 'b':
	...
	if (node_str) {
		...
		!strcmp("n-1", node_str) || !strcmp("localhost", node_str)) {
	...
	if (io[i].type == BPROC_IO_SOCKET) {
		...
		if (io[i].type == BPROC_IO_SOCKET) {
		    num_files++;
		    ((struct sockaddr_in *)&io[i].d.addr)->sin_port =
			addr.sin_port;
		}
	    break;
	}
	...
	if (rlim.rlim_max < num_files) rlim.rlim_max = num_files;
	
	if (setrlimit(RLIMIT_NOFILE, &rlim)) {
	    fprintf(stderr,"WARNING: Failed to up RLIMIT_NOFILE to %ld\n",
		    (long) rlim.rlim_cur);
	}
    }
	...
	if (start_accepter(sockfd))
	    exit(1);
    if (time_cmd) gettimeofday(&start, 0);
    r = start_processes(io, 3, progname, cmd_argv);
    if (sockfd != -1) stop_accepter();
    if (r)
	exit(1);
    
    if (sockfd != -1) forward_io(sockfd, flags, io, 3, accept_fdlist,
				 accept_nfds);
	...
	if (WIFEXITED(status)) {
	    ...
	    fprintf(stderr, "bpsh: Child process exited abnormally.\n");
	    max_exit = 255;	/* a segfault or something */
	}
    }
    if (time_cmd) {
