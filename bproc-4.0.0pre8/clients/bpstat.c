...
void print_nodes_compact(struct bproc_node_info_t *node, int num) {
	...
	for (j = i; j < num && node_same_conf(&node[i], &node[j]); j++) {
		...
		*orig = 0;
	    ...
	    for (k=j+1; k < num && node_same_conf(&node[j], &node[k]) &&
...
void print_node_number(char *_arg) {
	...
	if (bproc_nodeaddr(i, (struct sockaddr *) &addr, &sz)) {
	    fprintf(stderr, "Error getting node address for node %d: %s\n",
		    i, strerror(errno));
	    exit(1);
	}
...
void grok_ps() {
    ...
    /* Output the header */
    fputs("NODE\t",stdout); fputs(line,stdout);
...
int main(int argc, char *argv[]) {
	...
	case 'v':
	    ...
	    exit(0);
	case 'l': print_func = print_nodes_long;    break;
	case 'c': print_func = print_nodes_compact; break;
	case 'a': print_func = print_nodes_address; break;
	case 's': print_func = print_nodes_state;   break;
	case 'n': print_func = print_nodes_number;  break;
	case 't': print_func = print_nodes_total;   break;
	    
	    /* Sorting flags */
	case 'R': sort_reverse = 1;             break;
	case 'N': sort_func = node_sort_id;     break;
	case 'S': sort_func = node_sort_status; break;
	case 'O': sort_func = node_sort_none;   break;
