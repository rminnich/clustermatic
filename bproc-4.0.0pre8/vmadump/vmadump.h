 ...
 *
 * $Id: vmadump.h,v 1.20 2004/10/09 01:08:00 mkdist Exp $
 *-----------------------------------------------------------------------*/
#ifndef _VMADUMP_H
#define _VMADUMP_H
...
struct vmadump_map_ctx {
     ...
     *  O_RDONLY.  The filename argument is a kernel space pointer.
     *  The semantics should be identical to:
     *    filp_open(filename, O_RDONLY);
     *
     * map_name:
...
extern long vmadump_freeze_proc(struct vmadump_map_ctx *, struct file *file,
				struct pt_regs *regs, int flags);
extern long vmadump_thaw_proc  (struct vmadump_map_ctx *, struct file *file,
				struct pt_regs *regs);
