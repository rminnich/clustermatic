...
static inline struct bproc_credentials_t *creds_ptr(void *ptr, int offset)
{
...
void move_find_data_src(struct move_t *mv1)
{
	...
	for (l = mv1->list.prev; l != &mv1->list && l != &moves; l = l->prev) {
		...
		if (msg->addr == mv1_msg->addr && msg->port == mv1_msg->port) {
...
struct loopctr_t *loop_findctr(void *id, unsigned char fromtype, int from)
{
	...
	for (l = loopctrs.next; l != &loopctrs; l = l->next) {
		...
		if (lc->id == id && lc->fromtype == fromtype
		    && lc->from == from)
			return lc;
	}
	return 0;
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "hnmplis", long_options, 0)) != -1) {
		switch (c) {
		case 'h':
			printf("Usage: %s [options] < tracefile\n"
