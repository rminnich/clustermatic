 ...
 *
 *  $Id: cmconf.c,v 1.1 2003/09/24 20:30:34 mkdist Exp $
 *--------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
	...
	for (opt = options; opt->tag; opt++) {
		...
		if ((conf->pass == -1 || opt->pass == conf->pass) &&
			...
			if (opt->maxargs != -1 && i - 1 > opt->maxargs) {
				fprintf(stderr,
					"%s:%d Too many arguments for %s. (max = %d)\n",
					conf->filename, conf->lineno, opt->tag,
					opt->maxargs);
				return -1;
			}
			if (i - 1 < opt->minargs) {
				fprintf(stderr,
					"%s:%d Too few arguments for %s. (min = %d)\n",
					conf->filename, conf->lineno, opt->tag,
					opt->minargs);
				return -1;
			}
...
int cmconf_process_line(struct cmconf_line *line, struct cmconf_option *options)
{
	struct cmconf conf;
	conf.filename = "";
	conf.lineno = 0;
...
struct cmconf_line *cmconf_next(struct cmconf *conf, struct cmconf_line *line)
{
