...
void msgtrace(struct bproc_request_t *req, char *src) {
	...
	fprintf(tracefile, "first=%d node=%d", req->bpr_proc[0].pid, req->bpr_proc[0].node);
	break;
#endif
    case BPROC_SYS_KILL:
	fprintf(tracefile, "pid=%d sig=%d", req->bpr_sig_pid, req->bpr_sig_info.si_signo);
	break;
    case BPROC_PTRACE:
	fprintf(tracefile, "%-8.8s pid=%d %08lx %08lx", ptrace_types[req->bpr_ptrace_req], (int)req->bpr_ptrace_pid,
		req->bpr_ptrace_addr, req->bpr_ptrace_data);
	break;
    }
    fprintf(tracefile, "\n");
    fflush(tracefile);
