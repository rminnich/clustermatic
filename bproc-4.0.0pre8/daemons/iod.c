...
static inline int buffer_max_write(struct buffer_t *b)
{
...
static inline int buffer_max_read(struct buffer_t *b)
{
	if (b->start + b->bytes >= b->size)
...
static inline int buffer_full(struct buffer_t *b)
{
	return b->bytes == b->size;
}

static inline int buffer_empty(struct buffer_t *b)
{
	return b->bytes == 0;
}

static inline void buffer_flush(struct buffer_t *b)
{
	b->bytes = 0;
}

static inline void *buffer_read_ptr(struct buffer_t *b)
{
	return b->buffer + b->start;
}

static inline void *buffer_write_ptr(struct buffer_t *b)
{
	return b->buffer + ((b->start + b->bytes) % b->size);
}

static inline void buffer_move_write(struct buffer_t *b, int offs)
{
	b->bytes += offs;
}

static inline void buffer_move_read(struct buffer_t *b, int offs)
{
	b->start = (b->start + offs) % b->size;
	b->bytes -= offs;
}
 ...
 *
 *-----------------------------------------------------------------------*/
static inline void iod_close(int *fd)
{
	if (*fd == -1)
		return;
	close(*fd);
...
{
	int r;
	r = read(c->in, buffer_write_ptr(&c->buffer),
		 buffer_max_write(&c->buffer));
	if (r == -1 || r == 0) {
...
{
	int r;
	r = write(c->out, buffer_read_ptr(&c->buffer),
		  buffer_max_read(&c->buffer));
	if (r == -1 || r == 0) {
		do_conn_close(c);
		return;
	}
	 ...
	 * emptying the buffer. */
	if (c->in == -1 && buffer_empty(&c->buffer))
		iod_close(&c->out);
}
...
void pickup_new_ioc(int iodfd)
{
	...
	/*** We're out of file descriptors... time to fork... ---------***/
		case EMFILE:
			...
			if (!list) {
				syslog(LOG_CRIT,
				       "iod: Aaiiee!! No connections but still got EMFILE!");
				exit(1);
			}
			pid = fork();
			if (pid < 0) {
	...
	/*** someone beat us to it, no problem... ---------------------***/
		...
		default:
			syslog(LOG_ERR,
			       "iod: get new ioc failed: %s  (ignoring)",
			       strerror(errno));
			return;
		}
	}
...
void iod_main(int iodfd)
{
	...
	/*** Prepare fd_sets ------------------------------------------***/
		...
		for (c = list; c; c = c->next) {
			if (c->out != -1 && !buffer_empty(&c->buffer))
				FD_SET(c->out, &wset);
			if (c->in != -1 && !buffer_full(&c->buffer))
				FD_SET(c->in, &rset);
		...
		if (r > 0) {
			if (FD_ISSET(iodfd, &rset))
				pickup_new_ioc(iodfd);

			/* Service connections */
			for (c = list; c; c = c->next) {
				if (c->out != -1 && FD_ISSET(c->out, &wset))
					do_conn_write(c);
				if (c->in != -1 && FD_ISSET(c->in, &rset))
					do_conn_read(c);
