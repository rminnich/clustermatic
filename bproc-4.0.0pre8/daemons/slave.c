...
static inline
void masq_send(struct request_t *req) {
    list_add_tail(&req->list, &reqs_to_masq);
}

static inline
void master_send(struct request_t *req) {
...
void do_slave_chroot(struct request_t *req) {
	...
	err = errno;
	syslog(LOG_ERR, "chroot to %s failed: %s",msg->path,strerror(errno));
    }
...
void do_node_reboot(struct request_t *req) {
    ...
    msg = bproc_msg(req);
    switch (msg->hdr.req) {
    case BPROC_NODE_REBOOT: cmd = RB_AUTOBOOT; cmd_name = "reboot"; break;
    case BPROC_NODE_HALT:   cmd = RB_HALT_SYSTEM; cmd_name = "halt"; break;
    case BPROC_NODE_PWROFF: cmd = RB_POWER_OFF; cmd_name = "power off"; break;
    default:
	syslog(LOG_ERR, "Unrecognized node state command: %d", msg->hdr.req);
	return;
    }
...
void set_running(struct conn_t *conn) {
	...
	syslog(LOG_ERR, "ioctl(BPROC_MASQ_SET_MYADDR: %s", strerror(errno));
	return;
    }
    addrsize = sizeof(addr);
	...
	syslog(LOG_ERR, "ioctl(BPROC_MASQ_SET_MASTERADDR: %s",strerror(errno));
	return;
    }
    if (verbose)
...
void reconnect(struct conn_t *conn, struct request_t *req) {
    ...
    if (msg->conn.raddr == INADDR_ANY ||
	msg->conn.raddr == INADDR_NONE)
...
int conn_msg_in(struct conn_t *c, struct request_t *req) {
    ...
    switch(hdr->req) {
    case BPROC_VERSION: {
	struct bproc_version_msg_t *msg = (struct bproc_version_msg_t *) hdr;

	if (version.magic != msg->vers.magic ||
	    version.arch  != msg->vers.arch  ||
	    strcmp(version.version_string, msg->vers.version_string) != 0) {
	    syslog(LOG_NOTICE, "BProc version mismatch.  slave=%s-%u-%d;"
		   " master=%s-%u-%d (%s)",
	...
	if (private_namespace == -1) {
	    private_namespace = msg->private_namespace ? 1 : 0;
	    if (private_namespace) {
		syslog(LOG_NOTICE, "Setting up private FS name space.");
		privatize_namespace();
	    }
	} else {
	    static int complained = 0;
	    if (private_namespace != msg->private_namespace && !complained) {
		complained = 1;
		syslog(LOG_WARNING, "private namespace option changed.  This "
	...
	if (msg->hdr.to != node_number) {
            ...
            syslog(LOG_NOTICE, "Setting node number to %d", node_number);
            ioctl(masqfd, BPROC_MASQ_SET_NODENUM, node_number);
        }
        if (msg->ping_timeout != ping_timeout) {
            ping_timeout = msg->ping_timeout;
            syslog(LOG_NOTICE, "Setting ping timeout to %d", ping_timeout);
        }

	/* Pass master list up to the slave manager */
	write(manager_fd, &msg->masters_size, sizeof(msg->masters_size));
	write(manager_fd, ((void *)msg) + msg->masters,
	      msg->masters_size * sizeof(struct bproc_master_t));
	free(req);
    } return 1;
	...
	if (c->state != CONN_CLOSING)
	    syslog(LOG_NOTICE, "Received EOF on non-closing connection.");
	conn_remove(c, 0);
	free(req);
	return 0;
...
void conn_read(struct conn_t *c) {
	...
	if (c->ireq) {
		...
		syslog(LOG_ERR, "read(slave) (%p): %s", c, strerror(errno));
	    }
	    if (r <= 0) {
		...
		if (hdr->size < sizeof(struct bproc_message_hdr_t) ||
		    ...
		    syslog(LOG_ERR, "Invalid message size %d master",
			   hdr->size);
		    conn_remove(c, 0);
		    return;
		}
...
int conn_write_refill(struct conn_t *c) {
	...
	case BPROC_RESPONSE(BPROC_MOVE): {
	    struct bproc_move_msg_t *move_msg = (struct bproc_move_msg_t *)msg;
	    if (move_msg->hdr.size >= sizeof(*move_msg) &&
		move_msg->addr == 0)
		move_msg->addr = c->laddr.sin_addr.s_addr;
	    } break;
	}
    }
...
void conn_write(struct conn_t *c) {
	...
	if (w < 0) {
	    ...
	    syslog(LOG_NOTICE, "write(master): %s", strerror(errno));
	}
	if (w <= 0) {
	    /*syslog(LOG_NOTICE, "lost connection to master");*/
	    conn_remove(c, 0);
...
int read_masq_request(void) {
	...
	if (size <= 0) {
	    if (size == 0 || errno == EAGAIN) return 0;
...
int write_masq_request(void) {
	...
	if (r != msg->size) {
	    syslog(LOG_CRIT, "bproc: slave write error: %s (ignoring)",
		   strerror(errno));
	}
...
void select_loop(void) {
	...
	if (r > 0) {
	    ...
	    for (l = clist.next; !list_empty(&clist) && l != &clist; l=next) {
		next = l->next;
		c = list_entry(l, struct conn_t, list);
	...
	if (r == 0) {
	    ...
	    syslog(LOG_NOTICE, "ping timeout - lost connection to master");
	    return;
	}
    }
}
...
void read_master_set(struct master_t *s) {
	...
	if (!group2) {
	    ...
	    syslog(LOG_ERR, "ERROR: address moved to a different group.");
...
void usage(char *arg0) {
    printf(
"Usage: %s [options] masteripaddr [port]\n"
...
int main(int argc, char *argv[]) {
	...
	case 'l':
		...
		if (strcasecmp(facilitynames[i].c_name, optarg) == 0) {
		    log_facility = facilitynames[i].c_val;
		    break;
		}
	    }
	    if (facilitynames[i].c_name) break;
	...
	case 'm':
	    ...
	    break;
	case 'r': auto_reconnect = 1; break;
	case 'v': verbose++; want_daemonize = 0; break;
	...
	case 's':
	    ...
	    addrp->sin_port = 0;
	    break;
	case 'e': log_stderr = 1; break;
	    break;

	default: exit(1);
	}
    }
    if (verbose && !want_daemonize) log_stderr = 1;
	...
	if (*check) {
	    syslog(LOG_ERR, "invalid port number: %s", argv[optind+1]);
	    exit(1);
	}
    } else
	port = DEFAULT_PORT;
	...
	reset_slave();
	sleep(RECONNECT_RETRY_INTERVAL);
	if (slave_setup(&server_addr, &local_addr) == -1) continue;
	select_loop();
    }
    exit(1);
#endif
