...
int bjs_job_runnable(struct bjs_job_t *j) {
	...
	for (i=0; i < j->pool->nnodes; i++) {
	    n = j->pool->nodes[i];
	    if (n < bjs_nids && bjs_node_idx[n]->up) upcount++;
	}
	...
	for (i=0; i < clean_set.size; i++) {
		...
		if (clean_set.node[i].node == plist[j].node) {
		    ...
		    /* Hack to not try to kill root-owned */
                    sprintf(procname, "/proc/%d", plist[j].pid);
...
int start_job_batch(struct bjs_job_t *j) {
	...
	if (set_job_io(j))
	    exit(1);
	execl(j->shell, j->shell, "-c", j->cmdline, 0);
	exit(1);
    }
    /* Parent */
    j->pid = pid;
...
int config_nodes_callback(struct cmconf *cnf, char **args) {
	...
	p->nodes = realloc_chk(p->nodes, sizeof(int) * (p->nnodes + ns2.size));
...
int update_machine_status(int status_fd) {
    ...
    if (change) {
	bproc_nodeset_to_bjs_nodes(&new_nodes,&nnodes,&new_node_idx,&nids,&ns);
	...
	if (bjs_nodes[i].up != status) {
		...
		for (j=0; j < conf.npools; j++)
		    for (k=0; k < conf.pools[j].nnodes; k++)
			if (conf.pools[j].nodes[k] == bjs_nodes[i].node) {
			    bjs_clean_node(bjs_nodes[i].node);
			    break;
			}
		change = 1;
	    }
...
int main(int argc, char *argv[]) {
	...
	FD_ZERO(&rset);
	FD_ZERO(&wset);
	FD_SET(bproc_notify_fd, &rset); maxfd = bproc_notify_fd;
	...
	if (r > 0) {
		...
		if (verbose) syslog(LOG_INFO, "BProc machine status change.");
