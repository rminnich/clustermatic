...
int main(int argc, char *argv[]) {
 ...
 again:
    ...
    if (update) printf("\33[H\33[J"); /* Clear the screen */
	...
	printf("Pool: %s   Nodes (total/up/free): %s/%s/%s\n", s->list->val,
	       sexp_nth(s, 1)->list->val,
	       sexp_nth(s, 1)->list->next->val,
	       sexp_nth(s, 1)->list->next->next->val);
#endif
	...
	for (s2 = sexp_nth(s, 2)->list; s2; s2 = s2->next) {
	    ...
	    printf("%*.*s %c %-*.*s %-*.*s",
		   ...
		   strtol(sexp_nth(s2, SJX_START)->val,0,0) ? 'R' : ' ',
		   cw[1], cw[1], user,
		   cw[2], cw[2], sexp_nth(s2, SJX_CMD)->val);
		
	    reqs = sexp_nth(s2, SJX_REQS);
	    len = 0;
	    for (s3 = reqs->list; s3; s3=s3->next) {
		len += sprintf(reqstr+len,"%s", s3->list->val);
		if (s3->list->next)
		    len += sprintf(reqstr+len, "=%s", s3->list->next->val);
    ...
    if (update && bjs_recv(&sx) == 0) goto again;
    return 0;
}
