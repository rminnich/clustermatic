...
void sexp_free(struct sexp_t *sx) {
    ...
    while (sp >= 0) {
	sx = stack[sp--];
	if (sx->next) PUSH(sx->next);
	if (sx->list) PUSH(sx->list);
	free(sx);
    }
}

void sexp_parser_reset(struct sexp_parser_state_t *s) {
    if (s->top)  sexp_free(s->top);
...
static inline
struct sexp_t *malloc_sx(struct sexp_parser_state_t *s, int size) {
    struct sexp_t *sx;

    /* check limits */
    if (s->max_bytes && (s->bytes + sizeof(*sx) + size) > s->max_bytes) {
	fprintf(stderr, "Max bytes exceeded. %d %d %d\n",
		(int) s->max_bytes, (int) s->bytes, (int) sizeof(*sx) + size);
	return 0;
    }
...
static inline
int complete_atom(struct sexp_parser_state_t *s) {
    struct sexp_t *sx;
...
static inline
int add_to_atom(struct sexp_parser_state_t *s, char c) {
    char *tmp;
...
int sexp_parser_parse(const char *str_, int len, struct sexp_t **sx_out,
	...
	case SE_ATOM:
	    if (s->sp == -1) {
		fprintf(stderr, "stack empty; no atom allowed here.\n");
		return -1;
	    }
	    if (c == '(' || c == ')' || isspace(c)) {
		s->state = SE_NONE;
...
int sexp_length(struct sexp_t *sx) {
    int ct = 0;
    for (sx = sx->list; sx ; sx=sx->next) ct++;
    return ct;
}
