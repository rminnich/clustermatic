...
int rarp_prepare_interface(struct rarp_if_t *ifc) {
    ...
    if (ifr.ifr_flags & IFF_LOOPBACK) return 0; /* Skip loop backs... */
...
int mk_rarp_request(char *my_hwaddr, char *bfr) {
	...
	if (errno == ENOSYS) { 
	    ...
	    log_print(LOG_ERROR, "Error retrieving BProc version: %s\n",
		      strerror(errno));
	}
    }
...
int rarp_recv(int fd) {
	...
	if (memcmp(data->tgt_eth, ifc->hwaddr, ETH_ALEN) == 0) {
	    log_print(LOG_INFO, "response: %s %02X:%02X:%02X:%02X:%02X:%02X ->"
		      " %d.%d.%d.%d/%d.%d.%d.%d\n", ifc->name,
	    ...
	    memcpy(&ifc->server_ip, &data->src_ip, sizeof(ifc->server_ip));
	    memcpy(&ifc->my_ip,     &data->tgt_ip, sizeof(ifc->my_ip));
	    memcpy(&ifc->netmask,   &data->netmask,sizeof(ifc->netmask));

	    ifc->bproc_port = ntohs(data->bproc_port);
	    ifc->mcast_port = ntohs(data->mcast_port);
	    memcpy(ifc->boot_file, data->boot_file, BOOTFILE_MAXLEN);
	    ifc->boot_file[BOOTFILE_MAXLEN] = 0;
	    log_print(LOG_INFO, "response: bproc=%d; mcast=%d; file=%s\n",
		      ifc->bproc_port, ifc->mcast_port, ifc->boot_file);
	    return -1;
	}
    }
    return 0;
...
int rarp_do_rarp(int delay, int timeout) {
	...
	while (r != 0) {
	    if (r == -1) {
		log_print(LOG_ERROR, "select: %s", strerror(errno));
		return -1;
	    }
...
int rarp_configure(struct rarp_if_t *ifc) {
    ...
    addr->sin_family = AF_INET;
    addr->sin_addr.s_addr = ifc->my_ip.s_addr | (0xffffffff & ~ifc->netmask.s_addr);
...
int rarp_init(void) {
	...
	if (sockfd == -1) {
	    log_print(LOG_ERROR, "socket(AF_PACKET, SOCK_DGRAM, 0x%x): %s",
...
int rarp_all(int configure, int delay, struct rarp_data_t *data) {
    ...
    sockfd = socket(AF_PACKET, SOCK_DGRAM, htons(ETH_P_RARP));
    if (sockfd == -1) fatal("socket: %s",strerror(errno));
