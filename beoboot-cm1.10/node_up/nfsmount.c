 ...
 */
static int find_kernel_nfs_mount_version(void)
{
...
static struct pmap *get_mountport(struct sockaddr_in *server_addr,
				  ...
				  long unsigned proto,
				  long unsigned port, int nfs_mount_version)
{
	...
	*mount_opts = (char *)&data;
	...
	if (nfsvers == 2) {
		...
		memcpy(data.root.data,
		       (char *)status.nfsv2.fhstatus_u.fhs_fhandle, NFS_FHSIZE);
#if NFS_MOUNT_VERSION >= 4
		...
		memcpy(data.root.data,
		       (char *)fhandle->fhandle3_val, fhandle->fhandle3_len);
