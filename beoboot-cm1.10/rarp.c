...
int rarp_send(int sockfd, const char *interface) {
	...
	console_log("RARP: interface: %-8s %02x:%02x:%02x:%02x:%02x:%02x\n",
		    interface,
		    (int) hwaddr[0], (int) hwaddr[1], (int) hwaddr[2],
    ...
    r = sendto(sockfd, packet, len, 0, (struct sockaddr *)&addr, sizeof(addr));
...
int rarp_send_all(int sockfd) {
	...
	if (ifr.ifr_flags & IFF_LOOPBACK) continue; /* Skip loop backs... */
...
int rarp_recv(int sockfd, struct rarp_data_t *user) {
     ...
     */
    if (r < sizeof(*arp) + 20) return 0;
    if (arp->ar_op != htons(ARPOP_RREPLY)) return 0;
	...
	if (memcmp(data->tgt_eth, &ifr.ifr_hwaddr.sa_data, ETH_ALEN) == 0) {
	    console_log("RARP: %-8s %02X:%02X:%02X:%02X:%02X:%02X ->"
			" %d.%d.%d.%d/%d.%d.%d.%d\n",
			de->d_name,
	    ...
	    memcpy(&user->server_ip, data->src_ip,  sizeof(user->server_ip));
	    memcpy(&user->my_ip,     data->tgt_ip,  sizeof(user->my_ip));
	    memcpy(&user->netmask,   data->netmask, sizeof(user->netmask));
	    ...
	    console_log("RARP: bproc=%d; file=%d; file=%s\n",
			user->bproc_port, user->file_port, user->boot_file);
	    result = 1;
	    break;
	}
...
int rarp(struct rarp_data_t *data) {
	...
	console_log("RARP: delay=%.1fs -> %.1fs backoff=%.2f"
		    ...
		    rarp_args.backoff, rarp_args.max_time, rarp_args.rand);
	delay = 0;
    }
    ...
    printf("%c\b", *spinner_ptr); fflush(stdout);
    spinner_ptr++; if (!*spinner_ptr) spinner_ptr = spinner;
	...
	if (r == -1) fatal("select: %s", strerror(errno));
	if (r == 1)  result = rarp_recv(sockfd, data);
