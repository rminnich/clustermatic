...
int save_old_setup(struct monte_boot_t *boot)
{
	...
	if (syscall(__NR_reboot, MONTE_MAGIC_1, MONTE_MAGIC_2, 1, boot->setup)
	    != 0) {
		fprintf(stderr,
			"Failed to grab real mode configuration data.\n");
		return -1;
	}
			    ...
			    const void *buffer, long size)
{
	...
	if (strncmp(stmp->signature, SETUP_SIG_VAL, strlen(SETUP_SIG_VAL)) != 0) {
		fprintf(stderr,
			"monte: Kernel image setup signature not found.\n");
		return -1;
	}
...
int load_elf_image(struct monte_boot_t *boot, const void *buffer, long size)
{
	...
	for (i = 0; i < ehdr->e_phnum; i++) {
		...
		kernimage =
		    region_size(region, phdr[i].p_memsz + (1024 * PAGE_SIZE));
			    ...
			    const void *buffer, long size)
{
	...
	if (buffer2 != buffer)
		free(buffer2);	/* free it if necessary */
	return err;
}
			    ...
			    const void *buffer, long size)
{
	...
	region =
	    region_new(boot, (void *)__pa(boot->param.entrypoint - 0x6000));
...
int monte_boot(struct monte_boot_t *boot)
{
	...
	for (i = 0; i < boot->param.nregions; i++) {
		printf("monte: region: %6ld pages at %p\n",
		       boot->regions[i].size / PAGE_SIZE,
		       boot->regions[i].destaddr);
	...
	sync();
	sync();
	return syscall(__NR_reboot, MONTE_MAGIC_1, MONTE_MAGIC_2, 0,
		       &boot->param);
}
