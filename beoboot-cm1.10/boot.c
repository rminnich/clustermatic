...
void *check_malloc(size_t bytes) {
    void *m;
    m = malloc(bytes);
    if (!m) fatal("Out of memory allocating %d bytes", bytes);
    return m;
}
...
int module_scan(void) {
	...
	if (len > 3 && strcmp(de->d_name + len - 3, ".ko") == 0) {
		...
		if (strncmp(alias, "alias=", 6) == 0)
		    count++;
		alias = module_modinfo_next(mod->map, mod->size, alias);
	    }
	    mod->aliases = malloc(sizeof(*mod->aliases) * (count + 1));
...
void module_remove_all(void) {
    ...
    while (install_seq > 0) {
		...
		if (delete_module(mod->loaded_name, 0) != 0) {
		    console_log("   delete module failed: %s\n",
				strerror(errno));
		}
		break;
	    }
...
int mod_config_callback(struct cmconf *conf, char **args) {
	...
	if (mod) {
		...
		console_log("WARNING: duplicate module arguments for %s "
			    "(using last set)\n", args[1]);
	    }
	    free(mod->args);
...
struct scan_key_t pci_scan_keys[] = {
    ...
    {"interface", "i",  "%02lx", PCI_PATH "/%s/class", "0x%*02lx%*02lx%02lx"},
    {0,0,0,0,0,  0,0}
};
...
struct scan_key_t usb_scan_keys[] = {
    ...
    {"devsubclass","dsc", "%02lx", USB_PATH "/%s/bDeviceSubClass",    "%02lx"},
    {"devproto",   "dp",  "%02lx", USB_PATH "/%s/bDeviceProtocol",    "%02lx"},
    {"ifclass",    "ic",  "%02lx", USB_PATH "/%s/bInterfaceClass",    "%02lx"},
    {"ifsubclass", "isc", "%02lx", USB_PATH "/%s/bInterfaceSubClass", "%02lx"},
...
int device_probe(struct scan_class_t *class) {
	...
	for (sk = class->keys; sk->alias_pattern; sk++) {
	    sk->valid = generic_read_sysfs(sk->file, de->d_name,
					   sk->file_pattern, &sk->value);
	}

	for (mod = modlist; mod; mod = mod->next) {
	    if (mod->aliases) {
		for (i=0; mod->aliases[i]; i++) {
		    if (compare_alias(class, mod->aliases[i]) == 0 &&
			!mod->installed) {
			modprobe(mod, 0);
...
void download_boot_image(void) {
	...
	fatal("boot image has wrong architecture.  expected %d; got %d\n",
	      (int) BEOBOOT_ARCH, (int) header->arch);

    bootimg += sizeof(*header); bootimglen -= sizeof(*header);
    ...
    bootimg += header->kernel_size; bootimglen -= header->kernel_size;

    if (header->flags & BEOBOOT_INITRD_PRESENT) {
	len = monte_load_linux_initrd(boot, bootimg, header->initrd_size);
	if (len == -1)
...
void do_phase2(void) {
    ...
    if (!mod) fatal("Failed to load the bproc module.\n");
    modprobe(mod, mod->args);
...
int main(int argc, char *argv[]) {
	...
	while ((c = getopt(argc, argv, "C:r:M:m:pRc")) != -1) {
	    ...
	    case 'c':
		if (configure_interface(&rarp_data)) {
		    fatal("Failed to configure interface\n");
		}
		break;
#if 0
	    case 'R':
