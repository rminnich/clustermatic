...
long get_zImage_offset(char *filename) {
    ...
    if (memcmp(buffer, ZSIG, ZSIGLEN) != 0) goto bail;
...
int main(int argc, char *argv[]) {
	...
	if (offset != -1) {
	    ...
	    do {ch=fgetc(file);} while(ch && ch!= '=');/* Skip to the '=' */
	    ch = fgetc(file); while (ch){fputc(ch,stdout); ch = fgetc(file);}
	    fputc('\n',stdout);
	    continue;
	}
	...
	if (uncompress_kernel(argv[i]) == 0) {
		...
		/* XXX This will probably barf in the cross-architecture
		 * case... */
		fclose(file); file = fopen(tempfilename, "r");
		fseek(file, offset, SEEK_SET);
		fread(&buf, 1, sizeof(buf), file);
		puts(buf.release);
		fclose(file);
	...
	if (offset != -1) {
	    fseek(file, offset, SEEK_SET);
	    ch=fgetc(file);while(ch&&ch!=' '){fputc(ch,stdout);ch=fgetc(file);}
	    fputc('\n',stdout);
	    continue;
	}
