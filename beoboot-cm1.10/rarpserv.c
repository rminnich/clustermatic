...
int setup_socket(struct interface_t *ifc) {
	...
	syslog(LOG_ERR, "ioctl(SIOCGIFHWADDR,%s): %s", ifc->name, strerror(errno));
	close(sock);
	return -1;
    }
	...
	syslog(LOG_ERR, "ioctl(SIOCGIFADDR,%s): %s", ifc->name, strerror(errno));
	close(sock);
	return -1;
    }
	...
	syslog(LOG_ERR, "ioctl(SIOCGIFNETMASK,%s): %s", ifc->name, strerror(errno));
	close(sock);
	return -1;
    }
...
struct rarp_entry *add_node(int node_num) {
	...
	rarp_tmp = realloc(tc.rarp_list,
			   new_size * sizeof(*tc.rarp_list) * ALLOC_CHUNK);
	if (!rarp_tmp) {
...
int add_node_ip(int node_num, struct in_addr addr) {
    ...
    if (!(tmp = realloc(ent->ipaddr, (ent->nipaddr+1)*sizeof(*ent->ipaddr)))) {
	syslog(LOG_ERR, "Out of memory.");
	return -1;
    }
    ent->ipaddr = tmp;
...
int add_node_eth(int node_num, unsigned char *mac) {
    ...
    if (!(tmp=realloc(ent->ethaddr,sizeof(*ent->ethaddr)*(ent->nethaddr+1)))){
	syslog(LOG_ERR, "Out of memory.");
	return -1;
    }
    ent->ethaddr = tmp;
...
int rarp_serv_config_ip_range(struct cmconf *conf, char **args) {
	...
	syslog(LOG_ERR, "IP range overlaps allocated IP addresses: %s %s",
	       args[0], args[1]);
	return -1;
    }
...
int rarp_serv_config_node(struct cmconf *conf, char **args) {
	...
	for (i=0; i < ETH_ALEN; i++) {
	    if (sscanf(p, "%2x%n%*[ :.]%n", &val, &bytes, &bytes) < 1) {
		syslog (LOG_ERR, "Invalid hardware address: %s\n", *args);
		return -1;
	    }
	    p += bytes;
	    mac[i] = val;
...
int rarp_serv_config_bootfile_callback(int node, void *data) {
    ...
    rarp = find_node(node);
    if (!rarp) {
	syslog(LOG_WARNING, "bootfile node number (%d): no such node.", node);
	return 0;
    }
...
int rarp_setup(char *configfile, char *new_node_file) {
	...
	if (first)
	    ...
	    syslog(LOG_ERR, "Config reload failed; keeping old config.");
	return -1;
    }
    first = 0;
...
void send_response(struct interface_t *ifc, struct rarp_entry *entry,
    ...
    struct arpdata *data = (struct arpdata *) (packet+sizeof(struct arphdr));
    ...
    if (r == -1) {
	if (errno == ENOBUFS) return; /* Ignore this... */
...
void rarp_recv(struct interface_t *ifc) {
    ...
    if (r < sizeof(*arp) + 20) return;
    if (arp->ar_op != htons(ARPOP_RREQUEST)) return;
	...
	if (memcmp(&data->version.bproc_magic, conf.version.bproc_magic,
	    ...
	    if (memcmp(&data->version, &conf.version, sizeof(conf.version))) {
		/* Better to complain to much than too little here.
		 * We should throttle this or something */
		if (verbose) {
		    syslog(LOG_ERR, "Ignoring %02X:%02X:%02X:"
			   ...
			   conf.version.version_string, conf.version.magic,
			   conf.version.arch,
			   data->version.version_string, data->version.magic,
			   data->version.arch);
		}
		return; /* mismatch */
	    }
		...
		if (verbose) {
		    printf("Ignoring %02X:%02X:%02X:%02X:%02X:%02X due to "
			   " architecture mismatch.  me=%d  remote=%d\n",
			   (int)data->tgt_eth[0], (int)data->tgt_eth[1],
			   (int)data->tgt_eth[2], (int)data->tgt_eth[3],
			   (int)data->tgt_eth[4], (int)data->tgt_eth[5],
			   conf.version.arch, data->version.arch);
		}
		return;
	    }
	}
	...
	for (j=0; j < entry->nethaddr; j++)
	    if (memcmp(entry->ethaddr + j, data->tgt_eth, ETH_ALEN) == 0) {
		...
		for (k=0; k < entry->nipaddr; k++) {
		    if ((entry->ipaddr[k].s_addr &
			 ifc->netmask.sin_addr.s_addr) == if_netaddr)
			break;
		}
		if (k == entry->nipaddr) {
		    syslog(LOG_ERR, "RARP:  MAC address match but no netmask"
			   " match for node %s", mac_to_str(data->tgt_eth));
		    break;	/* punt... */
		}
